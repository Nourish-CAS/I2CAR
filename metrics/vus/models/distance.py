...
class Euclidean:
    ...
    window: int, optional (default = length of input dataset)
        The length of the subsequence to be compaired
    Attributes
    ----------
    decision_scores_ : numpy array of shape (n_samples,)
        The outlier scores of the training dataset.
...
class SSA_DISTANCE:
    ...
    def measure(self, X2, X3, start_index):
        ...
        """       
        #linearization of dataset X2 and X3
        X2 = np.array(X2)
        X3 = np.array(X3)
        e = self.e
    ...
    def measure(self, A, B, start_index):
        ...
        for i in range(1, n):
            for j in range(1, m):
                ...
                # Keep dataset points in both time series
                C[2] = (
                    DP[i - 1, j - 1]
                    + Dlp(A[i], B[j])
                    + Dlp(A[i - 1], B[j - 1])
